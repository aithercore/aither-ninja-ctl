...
try {
	...
	if (is_array($dataKraken) && isset($dataKraken['error']) && (count($dataKraken['error']) == 0) && isset($dataKraken['result']) && is_array($dataKraken['result']) && isset($dataKraken['result']['XXBTZEUR']) && is_array($dataKraken['result']['XXBTZEUR']) && isset($dataKraken['result']['XXBTZEUR']['p']) && is_array($dataKraken['result']['XXBTZEUR']['p']) && isset($dataKraken['result']['XXBTZEUR']['p'][1])) {
		$tp["eurobtc"] = array(
			"StatValue"  => $dataKraken['result']['XXBTZEUR']['p'][1],
			"LastUpdate" => time(),
			"Source"     => "kraken",
		);
...
if ($res !== false) {
	$res = json_decode($res, true);
	//  var_dump($res);
	if (($res !== false) && is_array($res) && (count($res) > 0) && array_key_exists('BTC_DASH', $res) && is_array($res["BTC_DASH"]) && array_key_exists("last", $res["BTC_DASH"])) {
		$tp["btcdrk"] = array(
			"StatValue"  => $res["BTC_DASH"]["last"],
			"LastUpdate" => time(),
			"Source"     => "poloniex",
		);
...
if ($res !== false) {
	...
	if (($res !== false) && is_array($res) && array_key_exists('pair', $res) && ($res["pair"] == "XBTUSD") && array_key_exists('lastPrice', $res) && array_key_exists('serverTimeUTC', $res)) {
		...
		$tp["usdbtc"] = array(
			"StatValue"  => floatval($res["lastPrice"]),
...
if ($res !== false) {
	...
	if (($res !== false) && is_array($res) && array_key_exists('symbol', $res) && ($res['symbol'] == 'dash') && array_key_exists('timestamp', $res)) {
		...
		if (array_key_exists('position', $res)) {
			$tp["marketcappos"] = array(
				"StatValue"  => $res["position"],
				"LastUpdate" => intval($res['timestamp']),
		...
		if (array_key_exists('change', $res)) {
			$tp["marketcapchange"] = array(
				"StatValue"  => $res["change"],
				"LastUpdate" => intval($res['timestamp']),
				"Source"     => "coinmarketcap",
			);
			$resdone ++;
		...
		if (array_key_exists('supply', $res)) {
			$tp["marketcapsupply"] = array(
				"StatValue"  => $res["supply"],
				"LastUpdate" => intval($res['timestamp']),
				"Source"     => "coinmarketcap",
			);
			$resdone ++;
		...
		if (array_key_exists('market_cap', $res) && is_array($res['market_cap'])) {
			if (array_key_exists('btc', $res['market_cap'])) {
				$tp["marketcapbtc"] = array(
					"StatValue"  => $res['market_cap']['btc'],
					"LastUpdate" => intval($res['timestamp']),
					"Source"     => "coinmarketcap",
				);
				$resdone ++;
			...
			if (array_key_exists('usd', $res['market_cap'])) {
				$tp["marketcapusd"] = array(
					"StatValue"  => $res['market_cap']['usd'],
					"LastUpdate" => intval($res['timestamp']),
					"Source"     => "coinmarketcap",
				);
				$resdone ++;
			...
			if (array_key_exists('eur', $res['market_cap'])) {
				$tp["marketcapeur"] = array(
					"StatValue"  => $res['market_cap']['eur'],
					"LastUpdate" => intval($res['timestamp']),
					"Source"     => "coinmarketcap",
				);
				$resdone ++;
...
foreach ($proposals as $proposal) {
	...
	if ($res !== false) {
		$res = json_decode($res, true);
		if (($res !== false) && is_array($res) && array_key_exists('status', $res) && ($res['status'] == 'ok') && array_key_exists('proposal', $res) && is_array($res["proposal"]) && array_key_exists('comments', $res) && is_array($res["comments"])) {
			$dwentry["proposal"] = $res["proposal"];
			foreach ($res["comments"] as $comment) {
				if ($comment !== false && is_array($comment) && array_key_exists('id', $comment) && is_string($comment["id"]) && array_key_exists('username', $comment) && is_string($comment["username"]) && array_key_exists('date', $comment) && is_string($comment["date"]) && array_key_exists('order', $comment) && is_int($comment["order"]) && array_key_exists('level', $comment) && array_key_exists('recently_posted', $comment) && is_bool($comment["recently_posted"]) && array_key_exists('posted_by_owner', $comment) && is_bool($comment["posted_by_owner"]) && array_key_exists('reply_url', $comment) && is_string($comment["reply_url"]) && array_key_exists('content', $comment) && is_string($comment["content"])) {
