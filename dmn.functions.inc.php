...
// Check if PID is running and is aitherd
function dmn_checkpid($pid) {
	if ($pid !== false) {
		$output = array();
		exec('ps -o comm -p ' . $pid, $output, $retval);
		if (($retval == 0) && (is_array($output)) && (count($output) >= 2)) {
			return (((strlen($output[1]) >= 5) && (substr($output[1], 0, 5) == 'aitherd')) || ((strlen($output[1]) >= 9) && (substr($output[1], 0, 9) == 'darkcoind')));
		} else {
			return false;
		}
	} else {
...
function dmn_getpid($uname, $testnet = false) {
	...
	} else if (file_exists(DMN_PID_PATH . $uname . "/.dashcore$testinfo/aitherd.pid") !== false) {
		$res = trim(file_get_contents(DMN_PID_PATH . $uname . "/.dashcore$testinfo/aitherd.pid"));
	...
	} else if (file_exists(DMN_PID_PATH . $uname . "/.dash$testinfo/aitherd.pid") !== false) {
		$res = trim(file_get_contents(DMN_PID_PATH . $uname . "/.dash$testinfo/aitherd.pid"));
...
// Get aitherd version from binary
function dmn_aitherdversion($dpath) {
