...
function dmn_getip($pid, $uname) {
	...
	$res = false;
	exec('netstat -ntpl | grep "tcp  " | egrep ":(1)?9999" | grep "' . $pid . '/darkcoind\|' . $pid . '/aitherd"', $output, $retval);
	if (isset($output[0])) {
...
function dmn_getstatus($aitherdinfo, $blockhash) {
	...
	if ($aitherdinfo !== false) {
		if (array_key_exists('version', $aitherdinfo)) {
			$res['version'] = $aitherdinfo['version'];
		}
		if (array_key_exists('protocolversion', $aitherdinfo)) {
			$res['protocol'] = $aitherdinfo['protocolversion'];
		}
		if (array_key_exists('difficulty', $aitherdinfo)) {
			$res['difficulty'] = $aitherdinfo['difficulty'];
		}
		if (array_key_exists('blocks', $aitherdinfo)) {
			$res['blocks'] = $aitherdinfo['blocks'];
		}
		if (array_key_exists('connections', $aitherdinfo)) {
			$res['connections'] = $aitherdinfo['connections'];
		}
		if (array_key_exists('testnet', $aitherdinfo) && $aitherdinfo['testnet']) {
			$res['testnet'] = 1;
		}
		$res['encryptedwallet'] = array_key_exists('unlocked_until', $aitherdinfo);
	}
	return $res;
}
...
function dmn_create($dmnpid, $ip, $forcename = '') {
	...
	touch("/home/$newuname/.aithercore/aither.conf");
	chmod("/home/$newuname/.dashcore", 0700);
	chmod("/home/$newuname/.aithercore/aither.conf", 0600);
	$conflist = array(
		'server=1',
	...
	$dashconf = implode("\n", $conflist);
	file_put_contents("/home/$newuname/.aithercore/aither.conf", $dashconf);
	echo "OK\n";
	echo "Setting ACL";
	...
	chown("/home/$newuname/.aithercore/aither.conf", $newuname);
	chgrp("/home/$newuname/.aithercore/aither.conf", $newuname);
	echo "OK\n";
...
function dmn_startstop($dmnpid, $todo, $testnet = false, $nodetype = 'masternode', $withreindex = false) {
	...
	foreach ($dmnpid as $node) {
		if (($node['testnet'] == $testnet) && ($node['type'] == $nodetype) && ($node['enabled'])) {
			$nodes[] = $node;
		}
	}
	if ($todo == 'start') {
	...
	foreach ($nodes as $nodenum => $node) {
		...
		$commands[] = array(
			"status"   => 0,
			"nodenum"  => $nodenum,
			"cmd"      => "$uname $todo " . $node['aitherd'] . $extra,
			"exitcode" => - 1,
			"output"   => '',
		);
	}
...
function dmn_status($dmnpid, $istestnet) {
		...
		if (($dmnpidinfo['pidstatus']) && ($dmnpidinfo['currentbin'] != '')) {
			...
			if (($dmnpidinfo['versionhandling'] == 3) && array_key_exists("mnbudgetshow", $dmnpidinfo) && is_array($dmnpidinfo["mnbudgetshow"])) {
				...
				if (array_key_exists("gobjectlist", $dmnpidinfo) && is_array($dmnpidinfo["gobjectlist"])) {
					...
					$dmnpid[$dmnnum]["gobjectlist"] = array(
						"proposals" => $gobjectproposals,
						"triggers"  => $gobjecttriggers,
					);
				}
			}
		}
	...
	foreach ($commands as $command) {
		if ($command['status'] != 2) {
			...
			if (((strlen($command['datatype']) > 18) && (substr($command['datatype'], 0, 18) == 'mnbudget-getvotes-')) || ((strlen($command['datatype']) > 17) && (substr($command['datatype'], 0, 17) == 'gobject-getvotes-'))) {
				$res = json_decode($res, true);
				if ($res === false) {
					xecho("Could not decode JSON from " . $command['file'] . "\n");
				}
	...
	$governancebudget  = array(
		false,
		false,
	);
	$governancenextsb  = array(
		false,
		false,
	);
	$spork = array();
	...
	foreach ($dmnpid as $dmnnum => $dmnpidinfo) {
		...
		if (($dmnpidinfo['pid'] !== false) && ($dmnpidinfo['currentbin'] != '')) {
			...
			// Parse status
			$aitherdinfo   = dmn_getstatus($dmnpidinfo['info'], $dmnpidinfo['blockhash']);
			$blocks      = $aitherdinfo['blocks'];
			$blockhash   = $aitherdinfo['blockhash'];
			$connections = $aitherdinfo['connections'];
			$difficulty  = $aitherdinfo['difficulty'];
			$protocol    = $aitherdinfo['protocol'];
			$version     = $aitherdinfo['version'];
			// Protocol
			...
			if ($version !== false) {
				...
				if ($dmnpidinfo['versionhandling'] <= 2) {
					...
					$mncurrentlist[$uname] = $mncurrentip . ":" . $aitherdinfo['testnet'];
					foreach ($dmnpidinfo['mnlastseen'] as $mnlsip => $data) {
						$mnlastseen[$uname][$mnlsip . ':' . $aitherdinfo['testnet']] = $data;
					}
					foreach ($dmnpidinfo['mnactiveseconds'] as $mnlsip => $data) {
						$mnactivesince[$uname][$mnlsip . ':' . $aitherdinfo['testnet']] = $data;
					}
					...
					foreach ($mnlist as $ip => $activetrue) {
						...
						$mnlistfinal["$ip:" . $aitherdinfo['testnet']][$uname] = array(
							'Status'   => $active,
							'PoS'      => $mnpose[$ip],
							'StatusEx' => $activetrue,
						);
					...
					if (is_array($mnvoteslist) && (count($mnvoteslist) > 0)) {
						foreach ($mnvoteslist as $ip => $vote) {
							$mnvoteslistfinal["$ip:" . $aitherdinfo['testnet']][$uname] = $vote;
						}
					}
					foreach ($mnpubkeylist as $data) {
						$mnpubkeylistfinal[$data["ip"] . ":" . $data["port"] . ":" . $aitherdinfo['testnet'] . ":" . $data["pubkey"]] = array(
							"MasternodeIP"   => $data["ip"],
							"MasternodePort" => $data["port"],
							"MNTestNet"      => $aitherdinfo['testnet'],
							"MNPubKey"       => $data["pubkey"],
						);
					}
					if (is_array($mndonationlist)) {
						foreach ($mndonationlist as $donatedata) {
							$mndonationlistfinal[$donatedata["ip"] . ":" . $donatedata["port"] . ":" . $aitherdinfo['testnet'] . ":" . $donatedata["pubkey"]] = array(
				...
				elseif ($dmnpidinfo['versionhandling'] >= 3) {
					...
					if ($dmnpidinfo['versionhandling'] == 3) {
						...
						if (is_array($dmnpidinfo['mnbudgetshow'])) {
							foreach ($dmnpidinfo['mnbudgetshow'] as $mnbudgetid => $mnbudgetdata) {
								if (array_key_exists($aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"], $mnbudgetshow)) {
									if (($mnbudgetshow[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["Yeas"] + $mnbudgetshow[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["Nays"] + $mnbudgetshow[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["Abstains"]) < ($mnbudgetdata["Yeas"] + $mnbudgetdata["Nays"] + $mnbudgetdata["Abstains"])) {
										$mnbudgetshow[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]                 = $mnbudgetdata;
										$mnbudgetshow[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]['BudgetId']     = $mnbudgetid;
										$mnbudgetshow[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["BudgetTesnet"] = $aitherdinfo['testnet'];
									}
								} else {
									$mnbudgetshow[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]                 = $mnbudgetdata;
									$mnbudgetshow[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]['BudgetId']     = $mnbudgetid;
								...
								if (array_key_exists("mnbudget-getvotes-" . $mnbudgetid, $dmnpidinfo)) {
									if (!array_key_exists($mnbudgetid, $mnbudgetvotes[$aitherdinfo['testnet']])) {
										$mnbudgetvotes[$aitherdinfo['testnet']][$mnbudgetid] = array();
									}
									if (is_array($dmnpidinfo["mnbudget-getvotes-" . $mnbudgetid])) {
										foreach ($dmnpidinfo["mnbudget-getvotes-" . $mnbudgetid] as $mnbudgetvotehash => $mnbudgetvotedata) {
											if (array_key_exists($mnbudgetvotehash, $mnbudgetvotes[$aitherdinfo['testnet']][$mnbudgetid])) {
												if ($mnbudgetvotes[$aitherdinfo['testnet']][$mnbudgetid][$mnbudgetvotehash]["nTime"] < $mnbudgetvotedata["nTime"]) {
													$mnbudgetvotes[$aitherdinfo['testnet']][$mnbudgetid][$mnbudgetvotehash] = $mnbudgetvotedata;
												}
											} else {
												$mnbudgetvotes[$aitherdinfo['testnet']][$mnbudgetid][$mnbudgetvotehash] = $mnbudgetvotedata;
											}
						...
						if (is_array($dmnpidinfo['mnbudgetprojection'])) {
							foreach ($dmnpidinfo['mnbudgetprojection'] as $mnbudgetid => $mnbudgetdata) {
								if (is_array($mnbudgetdata) && array_key_exists("Yeas", $mnbudgetdata) && array_key_exists("Nays", $mnbudgetdata) && array_key_exists("Abstains", $mnbudgetdata)) {
									if (array_key_exists($mnbudgetdata["Hash"], $mnbudgetprojection[$aitherdinfo['testnet']])) {
										if (($mnbudgetprojection[$aitherdinfo['testnet']][$mnbudgetdata["Hash"]]["Yeas"] + $mnbudgetprojection[$aitherdinfo['testnet']][$mnbudgetdata["Hash"]]["Nays"] + $mnbudgetprojection[$aitherdinfo['testnet']][$mnbudgetdata["Hash"]]["Abstains"]) < ($mnbudgetdata["Yeas"] + $mnbudgetdata["Nays"] + $mnbudgetdata["Abstains"])) {
											$mnbudgetprojection[$aitherdinfo['testnet']][$mnbudgetdata["Hash"]]                 = $mnbudgetdata;
											$mnbudgetprojection[$aitherdinfo['testnet']][$mnbudgetdata["Hash"]]['BudgetId']     = $mnbudgetid;
											$mnbudgetprojection[$aitherdinfo['testnet']][$mnbudgetdata["Hash"]]["BudgetTesnet"] = $aitherdinfo['testnet'];
										}
									} else {
										$mnbudgetprojection[$aitherdinfo['testnet']][$mnbudgetdata["Hash"]]                 = $mnbudgetdata;
										$mnbudgetprojection[$aitherdinfo['testnet']][$mnbudgetdata["Hash"]]['BudgetId']     = $mnbudgetid;
						...
						if (is_array($dmnpidinfo['mnbudgetfinal'])) {
							foreach ($dmnpidinfo['mnbudgetfinal'] as $mnbudgetid => $mnbudgetdata) {
								if (array_key_exists($aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"], $mnbudgetfinal) && array_key_exists("VoteCount", $mnbudgetfinal[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]])) {
									if (($mnbudgetfinal[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["VoteCount"]) < ($mnbudgetdata["VoteCount"])) {
										$mnbudgetfinal[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]                 = $mnbudgetdata;
										$mnbudgetfinal[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]['BudgetName']   = $mnbudgetid;
										$mnbudgetfinal[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]["BudgetTesnet"] = $aitherdinfo['testnet'];
									}
								} else {
									$mnbudgetfinal[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]                 = $mnbudgetdata;
									$mnbudgetfinal[$aitherdinfo['testnet'] . "-" . $mnbudgetdata["Hash"]]['BudgetName']   = $mnbudgetid;
					...
					elseif (($dmnpidinfo['versionhandling'] == 4) && ($dmnpidinfo['type'] != 'p2pool')) {
						...
						if (($governancenextsb[$aitherdinfo['testnet']] === false) || ($governancenextsb[$aitherdinfo['testnet']] > intval($dmnpidinfo['getgovernanceinfo']['nextsuperblock']))) {
							$governancenextsb[$aitherdinfo['testnet']] = intval($dmnpidinfo['getgovernanceinfo']['nextsuperblock']);
						}
						// Store the budget available in next superblock
						if (($governancebudget[$aitherdinfo['testnet']] === false) || ($governancebudget[$aitherdinfo['testnet']] > floatval($dmnpidinfo['getsuperblockbudget']))) {
							$governancebudget[$aitherdinfo['testnet']] = floatval($dmnpidinfo['getsuperblockbudget']);
						}
						// Parse proposals
						if (is_array($dmnpidinfo["gobjectlist"]) && is_array($dmnpidinfo["gobjectlist"]["proposals"])) {
							foreach ($dmnpidinfo["gobjectlist"]["proposals"] as $proposaldata) {
								if (array_key_exists($aitherdinfo['testnet'] . "-" . $proposaldata["hash"], $gobjectproposallist)) {
									if (($gobjectproposallist[$aitherdinfo['testnet'] . "-" . $proposaldata["hash"]]["gobject"]["YesCount"] + $gobjectproposallist[$aitherdinfo['testnet'] . "-" . $proposaldata["hash"]]["gobject"]["NoCount"] + $gobjectproposallist[$aitherdinfo['testnet'] . "-" . $proposaldata["hash"]]["gobject"]["AbstainCount"]) < ($proposaldata["gobject"]["YesCount"] + $proposaldata["gobject"]["NoCount"] + $proposaldata["gobject"]["AbstainCount"])) {
										$gobjectproposallist[$aitherdinfo['testnet'] . "-" . $proposaldata["hash"]]            = $proposaldata;
										$gobjectproposallist[$aitherdinfo['testnet'] . "-" . $proposaldata["hash"]]["Testnet"] = $aitherdinfo['testnet'];
									}
								} else {
									$gobjectproposallist[$aitherdinfo['testnet'] . "-" . $proposaldata["hash"]]            = $proposaldata;
									$gobjectproposallist[$aitherdinfo['testnet'] . "-" . $proposaldata["hash"]]["Testnet"] = $aitherdinfo['testnet'];
								...
								if (array_key_exists("gobject-getvotes-" . $proposaldata["hash"], $dmnpidinfo)) {
									if (!array_key_exists($proposaldata["hash"], $gobjectvotes[$aitherdinfo['testnet']])) {
										$gobjectvotes[$aitherdinfo['testnet']][$proposaldata["hash"]] = array();
									}
										...
										foreach ($dmnpidinfo["gobject-getvotes-" . $proposaldata["hash"]] as $gobjectvotehash => $gobjectvotedata) {
											...
											if ($signal == "FUNDING") {
												if ((substr($collateral, 0, 16) == "CTxIn(COutPoint(") && (substr($collateral, - 14) == "), scriptSig=)")) {
													...
													if (array_key_exists($mnoutputhash . "-" . $mnoutputindex, $gobjectvotes[$aitherdinfo['testnet']][$proposaldata["hash"]])) {
														if ($gobjectvotes[$aitherdinfo['testnet']][$proposaldata["hash"]][$mnoutputhash . "-" . $mnoutputindex]["nTime"] < $ntime) {
															$gobjectvotes[$aitherdinfo['testnet']][$proposaldata["hash"]][$mnoutputhash . "-" . $mnoutputindex] = array(
						...
						if (is_array($dmnpidinfo["gobjectlist"]) && is_array($dmnpidinfo["gobjectlist"]["triggers"])) {
							foreach ($dmnpidinfo["gobjectlist"]["triggers"] as $triggerdata) {
								if (array_key_exists($aitherdinfo['testnet'] . "-" . $triggerdata["hash"], $gobjecttriggerlist)) {
									if (($gobjecttriggerlist[$aitherdinfo['testnet'] . "-" . $triggerdata["hash"]]["gobject"]["YesCount"] + $gobjecttriggerlist[$aitherdinfo['testnet'] . "-" . $triggerdata["hash"]]["gobject"]["NoCount"] + $gobjecttriggerlist[$aitherdinfo['testnet'] . "-" . $triggerdata["hash"]]["gobject"]["AbstainCount"]) < ($triggerdata["gobject"]["YesCount"] + $triggerdata["gobject"]["NoCount"] + $triggerdata["gobject"]["AbstainCount"])) {
										$gobjecttriggerlist[$aitherdinfo['testnet'] . "-" . $triggerdata["hash"]]            = $triggerdata;
										$gobjecttriggerlist[$aitherdinfo['testnet'] . "-" . $triggerdata["hash"]]["Testnet"] = $aitherdinfo['testnet'];
									}
								} else {
									$gobjecttriggerlist[$aitherdinfo['testnet'] . "-" . $triggerdata["hash"]]            = $triggerdata;
									$gobjecttriggerlist[$aitherdinfo['testnet'] . "-" . $triggerdata["hash"]]["Testnet"] = $aitherdinfo['testnet'];
								...
								if (array_key_exists("gobject-getvotes-" . $triggerdata["hash"], $dmnpidinfo)) {
									if (!array_key_exists($triggerdata["hash"], $gobjectvotes[$aitherdinfo['testnet']])) {
										$gobjectvotes[$aitherdinfo['testnet']][$triggerdata["hash"]] = array();
									}
										...
										foreach ($dmnpidinfo["gobject-getvotes-" . $triggerdata["hash"]] as $gobjectvotehash => $gobjectvotedata) {
											...
											if ($signal == "FUNDING") {
												if ((substr($collateral, 0, 16) == "CTxIn(COutPoint(") && (substr($collateral, - 14) == "), scriptSig=)")) {
													$collateral = substr($collateral, 16, strlen($collateral) - 30);
													list($mnoutputhash, $mnoutputindex) = explode(", ", $collateral);
													if (array_key_exists($mnoutputhash . "-" . $mnoutputindex, $gobjectvotes[$aitherdinfo['testnet']][$triggerdata["hash"]])) {
														if ($gobjectvotes[$aitherdinfo['testnet']][$triggerdata["hash"]][$mnoutputhash . "-" . $mnoutputindex]["nTime"] < $ntime) {
															$gobjectvotes[$aitherdinfo['testnet']][$triggerdata["hash"]][$mnoutputhash . "-" . $mnoutputindex] = array(
																"MasternodeOutputHash"  => $mnoutputhash,
																"MasternodeOutputIndex" => intval($mnoutputindex),
																"VoteHash"              => $gobjectvotehash,
																"nTime"                 => intval($ntime),
					...
					foreach ($mn3listfull as $mn3output => $mn3data) {
						...
						if (array_key_exists($mn3output . "-" . $aitherdinfo['testnet'], $mninfo2)) {
							if ($mn3lastseen < $mninfo2[$mn3output . "-" . $aitherdinfo['testnet']]["MasternodeLastSeen"]) {
								$mninfo2[$mn3output . "-" . $aitherdinfo['testnet']]["MasternodeLastSeen"] = intval($mn3lastseen);
							}
							if ($mn3activeseconds < $mninfo2[$mn3output . "-" . $aitherdinfo['testnet']]["MasternodeActiveSeconds"]) {
								$mninfo2[$mn3output . "-" . $aitherdinfo['testnet']]["MasternodeActiveSeconds"] = intval($mn3activeseconds);
							}
							if ($mn3lastpaid > $mninfo2[$mn3output . "-" . $aitherdinfo['testnet']]["MasternodeLastPaid"]) {
								$mninfo2[$mn3output . "-" . $aitherdinfo['testnet']]["MasternodeLastPaid"] = intval($mn3lastpaid);
							}
						} else {
							$mninfo2[$mn3output . "-" . $aitherdinfo['testnet']] = array(
								"MasternodeProtocol"      => intval($mn3protocol),
						...
						$mnlist2final[$mn3output . "-" . $aitherdinfo['testnet']][$uname] = array(
							'Status'   => $active,
							'StatusEx' => $mn3status,
						);
					}
	...
	if (count($mnlistfinal) > 0) {
		...
		$estpayoutdaily = round(dmn_getpayout($mncountactive, $aitherdinfo['difficulty']), 2);
	} else {
		$estpayoutdaily = '???';
	}
...
if ($argc > 1) {
	$istestnet = 0;
	if ($argc > 2) {
		if (((strcasecmp($argv[1], 'status') == 0) || (strcasecmp($argv[1], 'start') == 0) || (strcasecmp($argv[1], 'stop') == 0) || (strcasecmp($argv[1], 'restart') == 0)) && ((strcasecmp($argv[2], 'testnet') == 0))) {
			$istestnet = 1;
		}
	}
...
if ($argc == 1) {
	...
	unlink($semfnam);
} elseif ((strcasecmp($argv[1], 'start') == 0) || (strcasecmp($argv[1], 'stop') == 0) || (strcasecmp($argv[1], 'restart') == 0)) {
	$todo    = strtolower($argv[1]);
	$testnet = ($argc > 2) && ($argv[2] == 'testnet');
	if (($argc > 3) && ((strcasecmp($argv[3], 'p2pool') == 0) || (strcasecmp($argv[3], 'masternode') == 0))) {
		$nodetype = $argv[3];
	} else {
		$nodetype = "masternode";
	}
