...
if ($response['http_code'] == 200) {
	...
	if ($response['http_code'] != 500) {
		...
		var_dump($result);
		if ($result !== false && isset($result['messages'])) {
...
if ($response['http_code'] == 200) {
	...
	if ($mnbal === false) {
		echo " Failed to JSON decode!\n";
		die(200);
	} elseif (!is_array($mnlist) || !array_key_exists('data', $mnbal) || !is_array($mnbal['data']) || !array_key_exists('balances', $mnbal['data']) || !is_array($mnbal['data']['balances']) || !array_key_exists('mainnet', $mnbal['data']['balances']) || !is_array($mnbal['data']['balances']['mainnet']) || !array_key_exists('testnet', $mnbal['data']['balances']) || !is_array($mnbal['data']['balances']['testnet'])) {
		echo " Incorrect data!\n";
		die(202);
	}
